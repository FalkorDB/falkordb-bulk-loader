...
from falkordb_bulk_loader.bulk_update import BulkUpdate, bulk_update, convert_cell
...
class TestConvertCell:
    ...
    def test_non_finite_floats_kept_as_strings(self):
        ...
        for token in (
            ...
            assert isinstance(
                result, str
            ), f"Expected str for {token!r}, got {type(result)}"
...
class TestTrickyCharacters:
    ...
    def test_empty_cell_guard(self):
        ...
        g = self.db_con.select_graph(graphname)
        result = g.query("MATCH (n:Item) RETURN n.id, n.score ORDER BY n.id")
